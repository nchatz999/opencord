  ...
  return (
    <div class={`relative overflow-hidden ${local.class ?? ""}`} style={{ height: `${height()}px` }}>
      <textarea
        ...
        rows={1}
        class={`w-full min-w-full px-2 py-2 text-base leading-6 whitespace-pre-wrap break-words bg-transparent resize-none outline-none overflow-y-auto placeholder:text-fg-subtle ${showOverlay() ? "text-transparent caret-fg-base selection:bg-accent-primary/30" : "text-fg-base"}`}
        style={{ "max-height": `${maxHeight()}px`, "field-sizing": "content" }}
        {...rest}
      />
      <Show when={showOverlay()}>
        <div
          class="absolute top-0 left-0 right-0 px-2 py-2 text-base leading-6 whitespace-pre-wrap break-words pointer-events-none text-fg-base"
