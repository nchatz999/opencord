...
#[derive(Clone)]
pub struct DefaultLoggerManager {
    sender: mpsc::UnboundedSender<LogEntry>,
    min_level: Arc<Mutex<LogLevel>>,
}

impl DefaultLoggerManager {
    pub fn new(min_level: LogLevel) -> (Self, mpsc::UnboundedReceiver<LogEntry>) {
        let (sender, receiver) = mpsc::unbounded_channel();
        (
            Self {
                sender,
                min_level: Arc::new(Mutex::new(min_level)),
            },
            receiver,
        )
    }

    pub fn default() -> (Self, mpsc::UnboundedReceiver<LogEntry>) {
        Self::new(LogLevel::Info)
    }
}

impl LoggerManager for DefaultLoggerManager {
    async fn log(&self, entry: LogEntry) -> Result<(), LoggerError> {
        ...
        self.sender
            .send(entry)
            .map_err(|_| LoggerError::WriteFailed)?;
