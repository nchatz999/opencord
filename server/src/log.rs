...
#[derive(Clone)]
pub struct LogService<M: LogManager, R: LogRepository> {
    manager: M,
    repository: R,
}

impl<M: LogManager, R: LogRepository> LogService<M, R> {
...
pub fn log_routes(
    log_service: LogService<TextLogManager, Postgre>,
...
async fn create_log_handler(
    State(service): State<LogService<TextLogManager, Postgre>>,
